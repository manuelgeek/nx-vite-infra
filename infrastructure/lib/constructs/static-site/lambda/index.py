# This handler is copied from @aws-cdk/aws-s3-deployment package.
#
# We extended its functionality by adding the `ReplaceValues` option which replaces
# some given patterns in the uploaded files. It is used to replace the placeholders
# for env variables added at build time with the actual values.
...
def handler(event, context):
    ...
    try:
        ...
        if request_type == "Update" or request_type == "Create":
            s3_deploy(s3_source_zips, s3_dest, user_metadata, system_metadata, prune, exclude, include, replace_values)
...
def s3_deploy(s3_source_zips, s3_dest, user_metadata, system_metadata, prune, exclude, include, replace_values):
    ...
    try:
        ...
        replace_values_in_files(contents_dir, replace_values)
...
def replace_values_in_files(contents_dir, replace_values):
